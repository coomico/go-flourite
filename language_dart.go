...
var dart = []languagePattern{
	...
	{expression: regexp.MustCompile(`^\s*(void\s)?main\(\)\s(async\s)?\{`), patternType: keywordFunction},
	...
	{expression: regexp.MustCompile(`\bnew\s(List|Map|Iterator|HashMap|Set)<\w+>\(\);$`), patternType: keywordVariable},
	{
	...
	{expression: regexp.MustCompile(`^\s*await\sfor`), patternType: keywordControl},
	{expression: regexp.MustCompile(`^\s*typedef\s.+\s=`), patternType: keywordControl},
